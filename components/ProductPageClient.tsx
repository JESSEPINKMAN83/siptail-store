...
export default function ProductPageClient({ product, related }: { product: Product; related: RelatedProduct[] }) {
  ...
  const addToCartBtn = (
    <button
      ...
      disabled={adding}
      className={`w-full py-4 rounded-xl font-bold text-base transition-all touch-manipulation min-h-[52px] ${
    ...
    <>
        ...
        <div className="grid grid-cols-2 gap-2">
          {product.images.slice(0, 4).map((url, i) => (
            <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-[#F4F4F4]">
              <ImgBox url={url} alt={`${product.name} view ${i + 1}`} />
              {i === 0 && (
        ...
        <div className="flex flex-col">
            ...
            {sizeOpen && (
              <div className="mt-1 border border-[#E5E7EB] rounded-xl overflow-hidden">
                {product.variants.map(v => (
                  <button
                    key={v.id}
                    onClick={() => { setSelectedVariant(v.id); setSizeOpen(false); }}
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="flex items-center border border-[#E5E7EB] rounded-xl overflow-hidden">
              <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-4 py-2 text-lg font-medium text-[#1A1A1A] hover:bg-gray-50 active:bg-gray-100 transition-colors touch-manipulation min-h-[44px] min-w-[44px]">−</button>
              <span className="px-5 py-2 text-sm font-semibold text-[#1A1A1A] border-x border-[#E5E7EB] min-w-[44px] text-center">{qty}</span>
              <button onClick={() => setQty(q => q + 1)} className="px-4 py-2 text-lg font-medium text-[#1A1A1A] hover:bg-gray-50 active:bg-gray-100 transition-colors touch-manipulation min-h-[44px] min-w-[44px]">+</button>
            </div>
          </div>
          ...
          <div className="border-t border-[#E5E7EB]">
            <Accordion title="Description">
              ...
              <ul className="mt-3 space-y-1 list-disc list-inside">
                ...
                <li>Available in 3 sizes for every breed</li>
            ...
            <Accordion title="Additional Info">
              ...
              <p className="mt-1">Weight: 180g (Medium). Dishwasher safe (tray only).</p>
            </Accordion>
            <Accordion title="Reviews (23)">
              <div className="space-y-3">
                {[
                  { name: "Sarah M.", stars: 5, text: "My lab loves it! The tray design is genius — no more wasteful pours." },
                  { name: "James T.", stars: 5, text: "Took this on a 10-mile hike. Zero leaks, easy one-hand use. Worth every penny." },
                ].map(r => (
                  <div key={r.name} className="border-b border-[#E5E7EB] pb-3">
            ...
            <Accordion title="Questions">
              <p>Have a question? Email us at hello@walkessentials.com and we'll reply within 24 hours.</p>
            </Accordion>
          </div>
      ...
      <div className="md:hidden">
        ...
        <div className="mb-3">
          <button
            onClick={() => setSizeOpen(!sizeOpen)}
            className="w-full flex items-center justify-between px-4 py-3.5 border border-[#E5E7EB] rounded-xl text-sm font-medium text-[#1A1A1A] touch-manipulation min-h-[52px]"
          >
            <span>Size: <span className="font-semibold">{sel?.label ?? "Select size"}</span></span>
            <svg className={`w-4 h-4 text-gray-400 transition-transform ${sizeOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
            ...
            <div className="mt-1 border border-[#E5E7EB] rounded-xl overflow-hidden">
              {product.variants.map(v => (
                <button key={v.id} onClick={() => { setSelectedVariant(v.id); setSizeOpen(false); }}
                  className={`w-full flex justify-between px-4 py-3.5 text-sm touch-manipulation min-h-[52px] ${selectedVariant === v.id ? "bg-[#1B4332] text-white" : "hover:bg-gray-50 text-[#1A1A1A]"}`}>
                  <span>{v.label}</span>
                  <span className={selectedVariant === v.id ? "text-green-200" : "text-[#6B7280]"}>{v.price}</span>
                </button>
              ))}
        ...
        <div className="flex items-center justify-between border border-[#E5E7EB] rounded-xl overflow-hidden mb-5">
          <button onClick={() => setQty(q => Math.max(1, q - 1))} className="flex-1 py-3 text-xl font-medium text-[#1A1A1A] hover:bg-gray-50 touch-manipulation min-h-[52px]">−</button>
          <span className="flex-1 text-center text-sm font-semibold text-[#1A1A1A] border-x border-[#E5E7EB] py-3">{qty}</span>
          <button onClick={() => setQty(q => q + 1)} className="flex-1 py-3 text-xl font-medium text-[#1A1A1A] hover:bg-gray-50 touch-manipulation min-h-[52px]">+</button>
        </div>
        ...
        <div className="border-t border-[#E5E7EB] mb-4">
          <Accordion title="Description">
            ...
            <ul className="mt-2 space-y-1 list-disc list-inside text-xs">
              ...
              <li>3 sizes for every breed</li>
            </ul>
          </Accordion>
          <Accordion title="Additional Info">
            <p>Material: BPA-free Tritan plastic + food-grade silicone tray. Dishwasher safe (tray only).</p>
          </Accordion>
          <Accordion title="Reviews (23)">
            <div className="space-y-3">
              {[
                { name: "Sarah M.", text: "My lab loves it! The tray design is genius." },
                { name: "James T.", text: "Zero leaks on a 10-mile hike. Worth every penny." },
              ].map(r => (
                <div key={r.name} className="border-b border-[#E5E7EB] pb-2">
          ...
          <Accordion title="Questions">
            <p>Email hello@walkessentials.com — reply within 24 hours.</p>
          </Accordion>
        </div>
          ...
          {related.map((rp) => (
            <Link key={rp.slug} href={`/products/${rp.slug}`}
              ...
              <div className="p-3 sm:p-4">
                ...
                <p className="font-bold text-[#1A1A1A] text-sm mb-3">{rp.price}</p>
                <button
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[#E5E7EB] px-4 py-3 flex items-center gap-3 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]">
        <div className="flex items-center border border-[#E5E7EB] rounded-xl overflow-hidden flex-shrink-0">
          <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-3 py-2 text-lg font-medium text-[#1A1A1A] hover:bg-gray-50 touch-manipulation min-h-[44px] min-w-[40px]">−</button>
          <span className="px-3 py-2 text-sm font-semibold text-[#1A1A1A] border-x border-[#E5E7EB] min-w-[36px] text-center">{qty}</span>
          <button onClick={() => setQty(q => q + 1)} className="px-3 py-2 text-lg font-medium text-[#1A1A1A] hover:bg-gray-50 touch-manipulation min-h-[44px] min-w-[40px]">+</button>
        </div>
        <button
          onClick={handleAddToCart}
          disabled={adding}
          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all touch-manipulation min-h-[48px] ${
            added ? "bg-green-600 text-white" : adding ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#1B4332] text-white hover:bg-[#2d5a3d] active:bg-[#143326]"
          }`}
        >
          {added ? "Added ✓" : adding ? "Adding..." : `Add to Cart — ${price}`}
        </button>
      </div>
    </>
  );
